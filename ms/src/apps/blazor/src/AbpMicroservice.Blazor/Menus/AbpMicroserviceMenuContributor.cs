...
namespace AbpMicroservice.Blazor.Menus;

public class AbpMicroserviceMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public AbpMicroserviceMenuContributor(IConfiguration configuration)
    {
    ...
    private Task ConfigureMainMenuAsync(MenuConfigurationContext context)
    {
        ...
        var l = context.GetLocalizer<AbpMicroserviceResource>();

        context.Menu.Items.Insert(
            0,
            new ApplicationMenuItem(
                AbpMicroserviceMenus.Home,
                l["Menu:Home"],
                "/",
                icon: "fas fa-home",
                order: 0
